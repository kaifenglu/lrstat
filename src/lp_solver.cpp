...
class Simplex {
 public:
  Simplex(const std::vector<std::vector<double>>& a,
      ...
      : m_(static_cast<int>(b.size())),
        ...
        d_(m_ + 2, std::vector<double>(n_ + 2)),
        basis_(m_), nonbasis_(n_ + 1) {
    for (int i = 0; i < m_; ++i) {
  ...
  double solve(std::vector<double>& x) {
    int row = 0;
    for (int i = 1; i < m_; ++i) {
      if (d_[i][n_ + 1] < d_[row][n_ + 1]) row = i;
    }
    if (d_[row][n_ + 1] < -1e-10) {
      ...
      if (std::find(basis_.begin(), basis_.end(), -1) != basis_.end()) {
        row = static_cast<int>(std::find(basis_.begin(), basis_.end(), -1) - basis_.begin());
        int col = 0;
        for (int j = 1; j <= n_ + 1; ++j) {
          if (d_[row][j] < d_[row][col] ||
              (std::fabs(d_[row][j] - d_[row][col]) < 1e-12 && nonbasis_[j] < nonbasis_[col])) {
            col = j;
          }
        }
        pivot(row, col);
    ...
    if (!simplex(2)) throw std::runtime_error("native LP is unbounded");
    x.assign(n_, 0.0);
    for (int i = 0; i < m_; ++i) {
      if (basis_[i] < n_) x[basis_[i]] = d_[i][n_ + 1];
    }
    return d_[m_][n_ + 1];
  }
 ...
 private:
  ...
  void pivot(int row, int col) {
    ...
    for (int j = 0; j < n_ + 2; ++j) {
      if (j != col) d_[row][j] *= inv;
    }
    for (int i = 0; i < m_ + 2; ++i) {
      if (i != row) d_[i][col] *= -inv;
    }
    d_[row][col] = inv;
  ...
  bool simplex(int phase) {
    ...
    while (true) {
      int col = -1;
      for (int j = 0; j <= n_; ++j) {
        if (phase == 2 && nonbasis_[j] == -1) continue;
        if (col == -1 || d_[objective][j] < d_[objective][col] - eps_ ||
            (std::fabs(d_[objective][j] - d_[objective][col]) <= eps_ &&
             nonbasis_[j] < nonbasis_[col])) col = j;
      }
      if (d_[objective][col] >= -eps_) return true;
      int row = -1;
      for (int i = 0; i < m_; ++i) {
        if (d_[i][col] <= eps_) continue;
        if (row == -1) {
          row = i;
        } else {
          ...
          if (lhs < rhs - eps_ ||
              (std::fabs(lhs - rhs) <= eps_ && basis_[i] < basis_[row])) row = i;
        }
      }
...
Rcpp::NumericVector lpMaxEqRcpp(const Rcpp::NumericVector& objective,
  ...
  for (int i = 0; i < m; ++i) {
    if (!std::isfinite(rhs[i])) throw std::invalid_argument("non-finite native LP input");
    b[i] = rhs[i];
    b[i + m] = -rhs[i];
    for (int j = 0; j < n; ++j) {
