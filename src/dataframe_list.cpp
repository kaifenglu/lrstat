...
void DataFrameCpp::push_back(std::vector<double> &&col,
                             const std::string &name) {
  if (containElementNamed(name))
    throw std::runtime_error("Column '" + name + "' already exists.");
  check_row_size(col.size(), name);
...
void DataFrameCpp::push_back(const std::vector<int> &col,
                             const std::string &name) {
  if (containElementNamed(name))
    throw std::runtime_error("Column '" + name + "' already exists.");
  check_row_size(col.size(), name);
...
void DataFrameCpp::push_back(const std::string &value,
                             const std::string &name) {
  size_t cur = nrows();
  if (cur == 0 && !names_.empty())
    throw std::runtime_error("Cannot push scalar when DataFrame has 0 rows");
...
void DataFrameCpp::push_back(const FlatMatrix &fm,
...
void DataFrameCpp::push_front(std::vector<double> &&col,
                              const std::string &name) {
  if (containElementNamed(name))
    throw std::runtime_error("Column '" + name + "' already exists.");
  check_row_size(col.size(), name);
...
void DataFrameCpp::push_front(const std::string &value,
                              const std::string &name) {
  size_t cur = nrows();
  if (cur == 0 && !names_.empty())
    throw std::runtime_error("Cannot push scalar when DataFrame has 0 rows");
...
ListCpp &ListCpp::get_list(const std::string &name) {
  ...
  if (auto p = std::get_if<ListPtr>(&var)) {
    if (!*p)
      throw std::runtime_error("List pointer is null for element '" + name +
                               "'");
    return **p;
  }
...
static inline void validate_row_idx(size_t nrows,
...
// Non-mutating subset: return a new FlatMatrix with rows/columns in specified
// ranges
FlatMatrix subset_flatmatrix(const FlatMatrix &fm, size_t row_start,
...
// Non-mutating subset for FlatArray: return a new FlatArray with rows in
// row_idx
...
FlatMatrix concat_flatmatrix(const FlatMatrix &fm1, const FlatMatrix &fm2) {
  ...
  // For each column, copy rows from fm1 then fm2 into the destination column
  // buffer
  for (size_t c = 0; c < ncol; ++c) {
    const double *src1 =
        fm1.data.empty() ? nullptr : fm1.data.data() + c * nrow1;
    const double *src2 =
        fm2.data.empty() ? nullptr : fm2.data.data() + c * nrow2;
    ...
    if (src1 && nrow1 > 0) {
      ...
      if (nrow1 > 0)
        std::fill(dst, dst + nrow1, 0.0);
    }
    if (src2 && nrow2 > 0) {
      std::memcpy(dst + nrow1, src2, nrow2 * sizeof(double));
    } else {
      if (nrow2 > 0)
        std::fill(dst + nrow1, dst + nrow, 0.0);
    }
  }
  return out;
}
...
void append_flatmatrix(std::vector<std::vector<double>> &fm1,
  ...
  // Check that all columns in fm1 have the same number of rows & capture that
  // count
  size_t nrow1 = 0;
  if (!fm1.empty()) {
    nrow1 = fm1[0].size();
...
void append_flatmatrix(std::vector<std::vector<double>> &fm1,
  ...
  if (fm1.size() != fm2.size()) {
    throw std::invalid_argument(
        "append_flatmatrix: number of columns must match");
  }
  ...
  for (size_t c = 0; c < ncol; ++c) {
    ...
    std::memcpy(dest_col.data() + old_size, fm2[c].data(),
                nrow2 * sizeof(double));
  }
}
...
FlatMatrix cols_to_flatmatrix(const std::vector<std::vector<double>> &cols) {
  ...
  for (size_t c = 1; c < ncol; ++c) {
    if (cols[c].size() != nrow) {
      throw std::invalid_argument(
          "cols_to_flatmatrix: inconsistent column lengths");
    }
  }
...
void flatmatrix_set_column(FlatMatrix &M, size_t col,
...
void intmatrix_set_column(IntMatrix &M, size_t col,
...
void sztmatrix_set_column(SztMatrix &M, size_t col,
...
// Visitor
struct RcppVisitor {
  template <typename T> Rcpp::RObject operator()(const T &x) const {
    return Rcpp::wrap(x);
  }
  ...
  Rcpp::RObject operator()(const std::vector<DataFrameCpp> &dfs) const {
    ...
    for (const auto &df : dfs)
      out.push_back(convertDataFrameCppToR(df));
    return out;
  }
  ...
  Rcpp::RObject operator()(const std::vector<ListPtr> &lists) const {
    ...
    for (const auto &p : lists) {
      if (!p)
        out.push_back(R_NilValue);
      else
        out.push_back(convertListCppToR(*p));
    }
    return out;
  }

  // Visitor for SztMatrix: convert std::size_t -> int, overflow -> NA (warn
  // once)
  ...
  // Handle vector<std::size_t> -> IntegerVector (with overflow -> NA and
  // warning)
  ...
  // Handle std::size_t scalar -> integer scalar (with overflow -> NA and
  // warning)
...
static bool convert_r_object_to_listcpp_variant(ListCpp &target,
                                                const std::string &name,
                                                const Rcpp::RObject &obj) {
  ...
  if (Rcpp::is<Rcpp::NumericVector>(obj)) {
    ...
    Rcpp::RObject dim_obj = nv.attr("dim");
    if (!dim_obj.isNULL()) {
      Rcpp::IntegerVector dim(dim_obj);
