    ...
    const int seed)
{
  ...
  for (size_t iter = 0; iter < niters; ++iter) {
    ...
    const double dropouts2 = sum2_totD[i2 + selected_arm];
    ...
    timeByArm(0, selected_arm) += sum1_T[i1];
    eventsByArm(0, selected_arm) += events1;
    dropoutsByArm(0, selected_arm) += dropouts1;
    ...
    for (size_t j = 1; j <= stop_k; ++j) {
      ...
      timeByArm(j, selected_arm) += sum2T[idx];
      eventsByArm(j, selected_arm) += devents1 + sum2_totE[idx];
      dropoutsByArm(j, selected_arm) += ddropouts1 + sum2_totD[idx];
...
Rcpp::List lrsim_seamless_Rcpp(
  ...
  auto out = lrsim_seamless_cpp(
    ...
    hrH0s, allocs, accrualT, accrualInt,
