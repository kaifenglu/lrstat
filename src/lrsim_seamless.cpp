    ...
    const int seed)
{
  ...
  const double* sum2T = sum2_analysisT.data();
  ...
  for (size_t iter = 0; iter < niters; ++iter) {
    ...
    // choose best arm (min log-rank among M arms at stage 1)
    size_t best_arm = 0;
    ...
    for (size_t k = 0; k < kMax; ++k) {
      for (size_t m = 0; m < M; ++m) {
        size_t idx = i2 + k * M + m;
        sum2_bestArm[idx] = static_cast<int>(best_arm + 1);
      }
    }

    // stage-1 totals (all active arms + control)
    const double events1 = sum1_E[i1 + M + 1];
    const double dropouts1 = sum1_D[i1 + M + 1];
    ...
    const double dropouts2 = sum2_totD[i2 + best_arm];
    ...
    const double ddropouts1 = dropouts1 - dropouts2;
    ...
    for (size_t k = 0; k < kMax; ++k) {
      const size_t idx = base_m + k * M;
      if (logRank[idx] < -criticalValues[k]) {
        stop_k = k;
        rejectByArm(k, M) += 1;
        if (k == 0) {
          for (size_t m = 0; m < M; ++m) {
            size_t idx2 = i2 + k * M + m;
            if (logRank[idx2] < -criticalValues[k]) {
              reject[idx2] = 1;
              rejectByArm(k, m) += 1;
            }
          }
    ...
    for (size_t j = 1; j <= stop_k; ++j) {
      ...
      dropoutsByArm(j, best_arm) += ddropouts1 + sum2_totD[idx];
