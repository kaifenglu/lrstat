...
std::vector<double>
nb_make_breaks(const std::vector<double> &piecewiseSurvivalTime,
...
template <class VGam>
static double nb_ex_integrand(const double x,    // analysis time
                              ...
                              const VGam &gamma, const double accrualDuration) {
...
template <class VGam>
static double
nb_info_integrand(const double x,    // analysis time
                  const double time, // calendar time of analysis
                  const double phi, const std::vector<double> &accrualTime,
                  const std::vector<double> &accrualIntensity,
                  const std::vector<double> &piecewiseSurvivalTime,
                  const double kappa, const double lambda,
                  const std::vector<double> &zero, const VGam &gamma,
...
ListCpp nbstat1cpp(const double time, const double rateRatioH0,
                   ...
                   const std::vector<double> &lambda2, const FlatMatrix &gamma1,
                   const FlatMatrix &gamma2, const double accrualDuration,
  ...
  std::vector<double> ndropouts(nstrata), ndropouts1(nstrata);
  std::vector<double> ndropouts2(nstrata);
  ...
  std::vector<double> vlogRate1H0(nstrata), vlogRate2H0(nstrata);
  std::vector<double> vlogRRH0(nstrata);
  ...
  for (std::size_t h = 0; h < nstrata; ++h) {
    ...
    if (hasDropout) {
      auto nd_row = nevent1(time, allocationRatioPlanned, accrualTime,
                            ...
                            gam2, zero, zero, accrualDuration, maxFollowupTime);
    ...
    auto f1_ex = [&](double x) -> double {
      return nb_ex_integrand(x, time, phi, accrualTime, accrualIntensity_frac,
                             piecewiseSurvivalTime, k1, lam1, zero, gam1,
                             accrualDuration);
    };
    auto f2_ex = [&](double x) -> double {
      return nb_ex_integrand(x, time, 1.0 - phi, accrualTime,
                             accrualIntensity_frac, piecewiseSurvivalTime, k2,
                             lam2, zero, gam2, accrualDuration);
    };
    ...
    auto f1_info = [&](double x) -> double {
      return nb_info_integrand(x, time, phi, accrualTime, accrualIntensity_frac,
                               piecewiseSurvivalTime, k1, lam1, zero, gam1,
                               accrualDuration);
    };
    auto f2_info = [&](double x) -> double {
      return nb_info_integrand(x, time, 1.0 - phi, accrualTime,
                               accrualIntensity_frac, piecewiseSurvivalTime, k2,
                               lam2, zero, gam2, accrualDuration);
    };
    ...
    if (nullVariance) {
        ...
        auto f = [&](double aval) -> double {
          ...
          auto f2_info_H0 = [&](double x) -> double {
            return nb_info_integrand(
                ...
                piecewiseSurvivalTime, k2, aval, zero, gam2, accrualDuration);
          };
      ...
      auto f1_info_H0 = [&](double x) -> double {
        return nb_info_integrand(x, time, phi, accrualTime,
                                 accrualIntensity_frac, piecewiseSurvivalTime,
                                 k1, lambda1H0[h], zero, gam1, accrualDuration);
      };
      auto f2_info_H0 = [&](double x) -> double {
        return nb_info_integrand(x, time, 1.0 - phi, accrualTime,
                                 accrualIntensity_frac, piecewiseSurvivalTime,
                                 k2, lambda2H0[h], zero, gam2, accrualDuration);
      };
...
ListCpp nbstatcpp(
    const std::vector<double> &time, const double rateRatioH0,
    const double allocationRatioPlanned, const std::vector<double> &accrualTime,
    const std::vector<double> &accrualIntensity,
    const std::vector<double> &piecewiseSurvivalTime,
    const std::vector<double> &stratumFraction,
    const std::vector<double> &kappa1, const std::vector<double> &kappa2,
    ...
    const bool fixedFollowup, const bool nullVariance) {
  ...
  if (!none_na(kappa1))
    throw std::invalid_argument("kappa1 must be provided");
  if (!none_na(kappa2))
    throw std::invalid_argument("kappa2 must be provided");
  if (!none_na(lambda1))
  ...
  for (double v : kappa1) {
    if (v < 0.0)
      throw std::invalid_argument("kappa1 must be non-negative");
  }
  for (double v : kappa2) {
    if (v < 0.0)
      throw std::invalid_argument("kappa2 must be non-negative");
  }
  ...
  for (size_t i = 0; i < k; ++i) {
    ListCpp out =
        nbstat1cpp(time[i], rateRatioH0, allocationRatioPlanned, accrualTime,
    ...
    // weighted geometric mean for rateRatio: exp(sum(w *
    // log(rateRatio_per_stratum)))
    ...
    if (!nullVariance) {
      ...
      double wg_log_l1H0 = 0.0, wg_log_l2H0 = 0.0, wg_log_l1 = 0.0;
      double wg_log_l2 = 0.0;
      ...
      zlogRRH0[i] =
          (logrr - std::log(rateRatioH0)) * std::sqrt(informationH0[i]);
...
//' @export
// [[Rcpp::export]]
Rcpp::List nbstat(const Rcpp::NumericVector &time = NA_REAL,
  ...
  auto out = nbstatcpp(time1, rateRatioH0, allocationRatioPlanned, accrualT,
                       accrualInt, pwSurvT, stratumFrac, kap1, kap2, lam1, lam2,
                       gam1, gam2, accrualDuration, followupTime, fixedFollowup,
                       nullVariance);
...
ListCpp nbpowercpp(
    ...
    const std::vector<double> &futilityRateRatio,
    const std::string &typeBetaSpending, const double parameterBetaSpending,
    const double rateRatioH0, const double allocationRatioPlanned,
    const std::vector<double> &accrualTime,
    const std::vector<double> &accrualIntensity,
    const std::vector<double> &piecewiseSurvivalTime,
    const std::vector<double> &stratumFraction,
    ...
    const std::vector<double> &gamma1, const std::vector<double> &gamma2,
    const double accrualDuration, const double followupTime,
    const bool fixedFollowup, const std::vector<double> &spendingTime,
    const double studyDuration, const bool nullVariance) {
  ...
  if (!missingFutilityBounds) {
    if (none_na(futilityBounds)) {
      ...
      if (futilityRateRatio.size() < kMax - 1) {
        throw std::invalid_argument(
            "Insufficient length for futilityRateRatio");
      }
      for (size_t i = 0; i < kMax - 1; ++i) {
        if (futilityRateRatio[i] <= 0) {
          throw std::invalid_argument("futilityRateRatio must be positive");
        }
      }
    }
  }
  ...
  ListCpp nb = nbstat1cpp(studyDuration1, rateRatioH0, allocationRatioPlanned,
                          accrualTime, accrualIntensity, piecewiseSurvivalTime,
                          stratumFraction, kappa1v, kappa2v, lambda1v, lambda2v,
                          gamma1x, gamma2x, accrualDuration, followupTime,
                          fixedFollowup, nullVariance);
  ...
  for (size_t i = 0; i < kMax - 1; ++i) {
    ...
    auto g = [&](double t) -> double {
      ListCpp nb1 =
          nbstat1cpp(t, rateRatioH0, allocationRatioPlanned, accrualTime,
                     accrualIntensity, piecewiseSurvivalTime, stratumFraction,
                     kappa1v, kappa2v, lambda1v, lambda2v, gamma1x, gamma2x,
  ...
  if (kMax > 1) {
    if (missingFutilityBounds && bsf == "none") {
      ...
      if (none_na(futilityBounds)) {
        ...
        for (size_t i = 0; i < kMax - 1; ++i) {
          futBounds[i] = (-std::log(futilityRateRatio[i] / rateRatioH0)) *
                         std::sqrt(I[i]) / w[i];
          if (futBounds[i] > critValues[i]) {
            throw std::invalid_argument(
                "futilityRateRatio values are too small to "
                "be compatible with criticalValues");
          }
        }
        futBounds[kMax - 1] = critValues[kMax - 1];
  ...
  if (!missingFutilityBounds || bsf == "none" || kMax == 1) {
    ...
    exit_probs = exitprobcpp(u, l, theta, I);
  } else {
...
Rcpp::List nbpower(
    ...
    const Rcpp::Nullable<Rcpp::NumericVector> futilityRateRatio = R_NilValue,
    const std::string &typeBetaSpending = "none",
    const double parameterBetaSpending = NA_REAL, const double rateRatioH0 = 1,
    const double allocationRatioPlanned = 1,
    const Rcpp::NumericVector &accrualTime = 0,
    const Rcpp::NumericVector &accrualIntensity = NA_REAL,
    const Rcpp::NumericVector &piecewiseSurvivalTime = 0,
    ...
    const double studyDuration = NA_REAL, const bool nullVariance = false) {
  ...
  std::vector<double> critValues, futBounds, futCP, futRateRatio;
  ...
  if (futilityRateRatio.isNotNull()) {
    futRateRatio = Rcpp::as<std::vector<double>>(futilityRateRatio);
  } else {
    futRateRatio = std::vector<double>(1, NaN);
  }
  ...
  ListCpp out = nbpowercpp(
      ...
      futBounds, futCP, futRateRatio, typeBetaSpending, parameterBetaSpending,
      rateRatioH0, allocationRatioPlanned, accrualT, accrualInt, piecewiseTime,
      stratumFrac, kap1, kap2, lam1, lam2, gam1, gam2, accrualDuration,
      followupTime, fixedFollowup, spendTime, studyDuration, nullVariance);
...
ListCpp nbsamplesizecpp(
    ...
    const std::vector<double> &futilityRateRatio,
    const std::string &typeBetaSpending, const double parameterBetaSpending,
    const std::vector<double> &userBetaSpending, const double rateRatioH0,
    const double allocationRatioPlanned, const std::vector<double> &accrualTime,
    ...
    const bool fixedFollowup, const std::vector<double> &spendingTime,
    const bool rounding, const bool nullVariance) {
  ...
  if (none_na(futilityRateRatio)) {
    for (size_t i = 0; i < kMax - 1; ++i) {
      futilityTheta[i] = -std::log(futilityRateRatio[i] / rateRatioH0);
    }
  }
  ...
  auto info_under_H1 = [allocationRatioPlanned, accrualTime,
                        piecewiseSurvivalTime, stratumFraction, kappa1v,
                        kappa2v, lambda1v, lambda2v, gamma1x, gamma2x,
                        fixedFollowup,
                        nstrata](double t, double accrDur, double fu,
                                 const std::vector<double> &accrInt) {
    ListCpp nb1 = nbstat1cpp(t, 1.0, allocationRatioPlanned, accrualTime,
                             accrInt, piecewiseSurvivalTime, stratumFraction,
                             kappa1v, kappa2v, lambda1v, lambda2v, gamma1x,
                             gamma2x, accrDur, fu, fixedFollowup, false);
  ...
  auto betadiff_under_H1 =
      [info_under_H1, kMax, infoRates, theta, rateRatioH0,
       allocationRatioPlanned, accrualTime, piecewiseSurvivalTime,
       stratumFraction, kappa1v, kappa2v, lambda1v, lambda2v, gamma1x, gamma2x,
       fixedFollowup, nullVariance, nstrata, critValues, futStopping,
       missingFutilityBounds, futBounds, futilityBounds, futilityCP,
       futilityRateRatio, beta, bsf, parameterBetaSpending, userBetaSpending,
    ...
    // --- compute maxInformation and theta via nbstat at study end ---
    ListCpp nb =
        nbstat1cpp(studyDuration1, rateRatioH0, allocationRatioPlanned,
                   accrualTime, accrInt, piecewiseSurvivalTime, stratumFraction,
                   kappa1v, kappa2v, lambda1v, lambda2v, gamma1x, gamma2x,
    ...
    for (size_t i = 0; i < kMax - 1; ++i) {
      ...
      // solve for analysis time where total information equals information1
      auto g = [info_under_H1, accrDur, fu, accrInt,
      ...
      ListCpp nb_i = nbstat1cpp(
          time[i], rateRatioH0, allocationRatioPlanned, accrualTime, accrInt,
          piecewiseSurvivalTime, stratumFraction, kappa1v, kappa2v, lambda1v,
          lambda2v, gamma1x, gamma2x, accrDur, fu, fixedFollowup, nullVariance);
  ...
  if (!nullVariance) {
    ...
    ListCpp design = getDesigncpp(
        beta, NaN, theta1, kMax, infoRates, effStopping, futStopping,
        critValues, alpha, asf, parameterAlphaSpending, userAlphaSpending,
        futBounds, futilityCP, futilityTheta, bsf, parameterBetaSpending,
    ...
    if (unknown == ACC_DUR) {
      auto f = [info_under_H1, followupTime, accrualIntensity,
                maxInformation](double x) -> double {
      ...
      if (!fixedFollowup && info_under_H1(accrualDuration, accrualDuration, 0.0,
                                          accrualIntensity) > maxInformation) {
        std::clog
            << "WARNING: Information at zero follow-up (end of enrollment) "
               "already exceeds target. Setting followupTime = 0 and "
               "finding minimal accrualDuration.\n";

        auto f = [info_under_H1, accrualIntensity, maxInformation](double x) {
        ...
        auto f = [info_under_H1, accrualDuration, accrualIntensity,
                  maxInformation](double x) -> double {
      ...
      auto f = [info_under_H1, accrualDuration, followupTime, accrualIntensity,
                maxInformation](double m) -> double {
    ...
    if (unknown == ACC_DUR) {
      auto f = [betadiff_under_H1, followupTime,
  ...
  if (nullVariance) {
    maxInformation = info_under_H1(studyDuration, accrualDuration, followupTime,
                                   accrualIntensity);
  }
  ...
  if (rounding) {
    double n0 =
        accrual1(studyDuration, accrualTime, accrualIntensity, accrualDuration);
    ...
    if (n - n0 > 1e-6) {
      ...
      if (!fixedFollowup) {
        ...
        auto h = [info_under_H1, accrualDuration, accrualIntensity,
                  maxInformation](double x) -> double {
          return info_under_H1(accrualDuration + x, accrualDuration, x,
                               accrualIntensity) -
                 maxInformation;
  ...
  ListCpp resultsH1 = nbpowercpp(
      ...
      futilityCP, futilityRateRatio, typeBetaSpending, parameterBetaSpending,
      rateRatioH0, allocationRatioPlanned, accrualTime, accrualIntensity,
      piecewiseSurvivalTime, stratumFraction, kappa1, kappa2, lambda1, lambda2,
      gamma1, gamma2, accrualDuration, followupTime, fixedFollowup,
  ...
  // followupTime (fu), and accrualIntensity (accrInt).
  auto info_minus_target_H0 =
      [rateRatioH0, allocationRatioPlanned, accrualTime, piecewiseSurvivalTime,
       stratumFraction, kappa1v, kappa2v, lambda1H0v, lambda2v, gamma1x,
       gamma2x, fixedFollowup, nstrata,
       maxInformation](double t, double accrDur, double fu,
                       const std::vector<double> &accrInt) {
        ListCpp nb1 =
            nbstat1cpp(t, rateRatioH0, allocationRatioPlanned, accrualTime,
                       accrInt, piecewiseSurvivalTime, stratumFraction, kappa1v,
                       kappa2v, lambda1H0v, lambda2v, gamma1x, gamma2x, accrDur,
                       fu, fixedFollowup, false);
  ...
  if (!fixedFollowup) {
    auto h_follow = [info_minus_target_H0, accrualDuration,
                     accrualIntensity](double x) -> double {
    ...
    if (h_follow(0.0) > 0.0) {
      ...
      auto h_accr = [info_minus_target_H0, accrualIntensity](double x) {
    ...
    auto h_accr = [info_minus_target_H0, followupTime,
                   accrualIntensity](double x) -> double {
      return info_minus_target_H0(x + followupTime, x, followupTime,
                                  accrualIntensity);
    };
  ...
  ListCpp resultsH0 = nbpowercpp(
      ...
      futilityCP, futilityRateRatio, typeBetaSpending, parameterBetaSpending,
      rateRatioH0, allocationRatioPlanned, accrualTime, accrualIntensity,
      piecewiseSurvivalTime, stratumFraction, kappa1, kappa2, lambda1H0,
      lambda2, gamma1, gamma2, accrualDuration, followupTime, fixedFollowup,
      spendingTime, studyDuration, nullVariance);
...
Rcpp::List nbsamplesize(
    ...
    const double rateRatioH0 = 1, const double allocationRatioPlanned = 1,
    const Rcpp::NumericVector &accrualTime = 0,
    const Rcpp::NumericVector &accrualIntensity = NA_REAL,
    const Rcpp::NumericVector &piecewiseSurvivalTime = 0,
    const Rcpp::NumericVector &stratumFraction = 1,
    ...
    const Rcpp::NumericVector &spendingTime = NA_REAL,
    const bool rounding = true, const bool nullVariance = false) {
  ...
  auto out = nbsamplesizecpp(
      ...
      critValues, alpha, typeAlphaSpending, parameterAlphaSpending, userAlpha,
      futBounds, futCP, futRateRatio, typeBetaSpending, parameterBetaSpending,
      userBeta, rateRatioH0, allocationRatioPlanned, accrualT, accrualInt,
      pwSurvT, stratumFrac, kap1, kap2, lam1, lam2, gam1, gam2, accrualDuration,
      followupTime, fixedFollowup, spendTime, rounding, nullVariance);
...
ListCpp nbpower1scpp(
    ...
    const std::vector<double> &futilityBounds,
    const std::vector<double> &futilityCP,
    const std::vector<double> &futilityRate,
    const std::string &typeBetaSpending, const double parameterBetaSpending,
    const double lambdaH0, const std::vector<double> &accrualTime,
    const std::vector<double> &accrualIntensity,
    const std::vector<double> &piecewiseSurvivalTime,
    const std::vector<double> &stratumFraction,
    const std::vector<double> &kappa, const std::vector<double> &lambda,
    const std::vector<double> &gamma, const double accrualDuration,
    const double followupTime, const bool fixedFollowup,
  ...
  bool missingCriticalValues = !none_na(criticalValues);
  bool missingFutilityBounds = !none_na(futilityBounds) &&
                               !none_na(futilityCP) && !none_na(futilityRate);
  ...
  if (!missingFutilityBounds) {
    if (none_na(futilityBounds)) {
      ...
      if (futilityRate.size() < kMax - 1) {
        throw std::invalid_argument("Insufficient length for futilityRate");
      }
      for (size_t i = 0; i < kMax - 1; ++i) {
        if (futilityRate[i] <= 0) {
          throw std::invalid_argument("futilityRate must be positive");
        }
      }
    }
  }
  ...
  if (std::isnan(lambdaH0))
    throw std::invalid_argument("lambdaH0 must be provided");
  if (lambdaH0 <= 0.0)
    throw std::invalid_argument("lambdaH0 must be positive");
  if (accrualTime[0] != 0.0)
    throw std::invalid_argument("accrualTime must start with 0");
  if (any_nonincreasing(accrualTime))
    throw std::invalid_argument("accrualTime should be increasing");
  ...
  if (std::fabs(sumf - 1.0) > 1e-12)
    throw std::invalid_argument("stratumFraction must sum to 1");
  if (!none_na(kappa))
    throw std::invalid_argument("kappa must be provided");
  if (!none_na(lambda))
  ...
  std::vector<double> time(kMax), I(kMax);
  std::vector<double> nsubjects(kMax), nevents(kMax), ndropouts(kMax);
  std::vector<double> exposure(kMax);

  // compute information using twin group trick
  std::vector<double> accrualInt2 = accrualIntensity;
  for (double &v : accrualInt2)
    v *= 2.0;

  ListCpp nb = nbstat1cpp(studyDuration1, 1, 1, accrualTime, accrualInt2,
                          piecewiseSurvivalTime, stratumFraction, kappav,
                          kappav, lambdav, lambdav, gammax, gammax,
                          accrualDuration, followupTime, fixedFollowup, false);
  ...
  for (size_t i = 0; i < kMax - 1; ++i) {
    ...
    auto g = [&](double t) -> double {
      ListCpp nb = nbstat1cpp(
          t, 1, 1, accrualTime, accrualInt2, piecewiseSurvivalTime,
          stratumFraction, kappav, kappav, lambdav, lambdav, gammax, gammax,
          accrualDuration, followupTime, fixedFollowup, false);
    ...
    ListCpp nb_i = nbstat1cpp(
        time[i], 1, 1, accrualTime, accrualInt2, piecewiseSurvivalTime,
        stratumFraction, kappav, kappav, lambdav, lambdav, gammax, gammax,
        accrualDuration, followupTime, fixedFollowup, false);
  ...
  if (kMax > 1) {
    if (missingFutilityBounds && bsf == "none") {
      ...
      if (none_na(futilityBounds)) {
        ...
        for (size_t i = 0; i < kMax - 1; ++i) {
          futBounds[i] =
              (-std::log(futilityRate[i] / lambdaH0)) * std::sqrt(I[i]);
          if (futBounds[i] > critValues[i]) {
            throw std::invalid_argument("futilityRate values are too small to "
                                        "be compatible with criticalValues");
          }
        }
        futBounds[kMax - 1] = critValues[kMax - 1];
...
Rcpp::List nbpower1s(
    ...
    const Rcpp::Nullable<Rcpp::NumericVector> futilityBounds = R_NilValue,
    const Rcpp::Nullable<Rcpp::NumericVector> futilityCP = R_NilValue,
    const Rcpp::Nullable<Rcpp::NumericVector> futilityRate = R_NilValue,
    const std::string &typeBetaSpending = "none",
    const double parameterBetaSpending = NA_REAL,
    const double lambdaH0 = NA_REAL, const Rcpp::NumericVector &accrualTime = 0,
    const Rcpp::NumericVector &accrualIntensity = NA_REAL,
    const Rcpp::NumericVector &piecewiseSurvivalTime = 0,
    const Rcpp::NumericVector &stratumFraction = 1,
  ...
  std::vector<double> critValues, futBounds, futCP, futRate;
  if (criticalValues.isNotNull()) {
    critValues = Rcpp::as<std::vector<double>>(criticalValues);
  } else {
    critValues = std::vector<double>(1, NaN);
  ...
  if (futilityRate.isNotNull()) {
    futRate = Rcpp::as<std::vector<double>>(futilityRate);
  } else {
    futRate = std::vector<double>(1, NaN);
  }

  ListCpp out = nbpower1scpp(
      static_cast<size_t>(kMax), infoRates, effStopping, futStopping,
      critValues, alpha, typeAlphaSpending, parameterAlphaSpending, userAlpha,
      futBounds, futCP, futRate, typeBetaSpending, parameterBetaSpending,
      lambdaH0, accrualT, accrualInt, pwSurvT, stratumFrac, kap, lam, gam,
      accrualDuration, followupTime, fixedFollowup, spendTime, studyDuration);
...
ListCpp nbsamplesize1scpp(
    ...
    const std::vector<double> &futilityRate,
    const std::string &typeBetaSpending, const double parameterBetaSpending,
    const std::vector<double> &userBetaSpending, const double lambdaH0,
    const std::vector<double> &accrualTime,
    std::vector<double> &accrualIntensity,
    const std::vector<double> &piecewiseSurvivalTime,
    const std::vector<double> &stratumFraction,
    const std::vector<double> &kappa, const std::vector<double> &lambda,
    const std::vector<double> &gamma, double accrualDuration,
    double followupTime, const bool fixedFollowup,
  ...
  std::vector<double> futilityTheta(kMax - 1, NaN);
  if (none_na(futilityRate)) {
    for (size_t i = 0; i < kMax - 1; ++i) {
      futilityTheta[i] = -std::log(futilityRate[i] / lambdaH0);
    }
  }

  // get design under H1 (delegates much work)
  ListCpp design =
      getDesigncpp(beta, NaN, theta1, kMax, infoRates, effStopping, futStopping,
                   critValues, alpha, asf, parameterAlphaSpending,
                   userAlphaSpending, futBounds, futilityCP, futilityTheta, bsf,
                   parameterBetaSpending, userBetaSpending, spendTime, 1.0);
  ...
  // Helper: compute information under H1 given accrualDuration (accrDur),
  // followupTime (fu), and accrualIntensity (accrInt).
  auto info_minus_target_H1 =
      [accrualTime, piecewiseSurvivalTime, stratumFraction, kappav, lambdav,
       gammax, fixedFollowup, nstrata,
       maxInformation](double t, double accrDur, double fu,
                       const std::vector<double> &accrInt) {
        // compute information using twin group trick
        std::vector<double> accrInt2 = accrInt;
        for (double &v : accrInt2)
          v *= 2.0;

        ListCpp nb =
            nbstat1cpp(t, 1, 1, accrualTime, accrInt2, piecewiseSurvivalTime,
                       stratumFraction, kappav, kappav, lambdav, lambdav,
                       gammax, gammax, accrDur, fu, fixedFollowup, false);
  ...
  if (unknown == ACC_DUR) {
    auto f = [info_minus_target_H1, followupTime, accrualIntensity](double x) {
      return info_minus_target_H1(x + followupTime, x, followupTime,
                                  accrualIntensity);
    };
    ...
    if (!fixedFollowup && info_minus_target_H1(accrualDuration, accrualDuration,
                                               0.0, accrualIntensity) > 0) {
      std::clog << "WARNING: Information at zero follow-up (end of enrollment) "
                   "already exceeds target. Setting followupTime = 0 and "
                   "finding minimal accrualDuration.\n";
      ...
      auto f = [info_minus_target_H1, accrualDuration,
                accrualIntensity](double x) -> double {
    ...
    auto f = [info_minus_target_H1, accrualDuration, followupTime,
              accrualIntensity](double m) -> double {
      std::vector<double> scaled = accrualIntensity;
      for (double &v : scaled)
        v *= m;
  ...
  if (rounding) {
    ...
    if (n - n0 > 1e-6) {
      ...
      if (!fixedFollowup) {
        // variable follow-up: adjust follow-up time to match maxInformation
        auto h = [info_minus_target_H1, accrualDuration,
                  accrualIntensity](double x) -> double {
          return info_minus_target_H1(accrualDuration + x, accrualDuration, x,
                                      accrualIntensity);
        };
  ...
  ListCpp resultsH1 = nbpower1scpp(
      kMax, infoRates, effStopping, futStopping, critValues, alpha,
      typeAlphaSpending, parameterAlphaSpending, userAlphaSpending, futBounds,
      futilityCP, futilityRate, typeBetaSpending, parameterBetaSpending,
      lambdaH0, accrualTime, accrualIntensity, piecewiseSurvivalTime,
      stratumFraction, kappa, lambda, gamma, accrualDuration, followupTime,
      fixedFollowup, spendingTime, studyDuration);
  ...
  // followupTime (fu), and accrualIntensity (accrInt).
  auto info_minus_target_H0 =
      [accrualTime, piecewiseSurvivalTime, stratumFraction, kappav, lambdaH0v,
       gammax, fixedFollowup, nstrata,
       maxInformation](double t, double accrDur, double fu,
                       const std::vector<double> &accrInt) {
        // compute information using twin group trick
        ...
        ListCpp nb =
            nbstat1cpp(t, 1, 1, accrualTime, accrInt2, piecewiseSurvivalTime,
                       stratumFraction, kappav, kappav, lambdaH0v, lambdaH0v,
                       gammax, gammax, accrDur, fu, fixedFollowup, false);
  ...
  if (!fixedFollowup) {
    ...
    if (h_follow(0.0) > 0.0) {
      ...
      auto h_accr = [info_minus_target_H0,
                     accrualIntensity](double x) -> double {
        return info_minus_target_H0(x, x, 0.0, accrualIntensity);
      };
  ...
  ListCpp resultsH0 = nbpower1scpp(
      ...
      futilityCP, futilityRate, typeBetaSpending, parameterBetaSpending,
      lambdaH0, accrualTime, accrualIntensity, piecewiseSurvivalTime,
      stratumFraction, kappa, lambdaH0v, gamma, accrualDuration, followupTime,
      fixedFollowup, spendingTime, studyDuration);
...
Rcpp::List nbsamplesize1s(
    ...
    const Rcpp::NumericVector &kappa = NA_REAL,
    const Rcpp::NumericVector &lambda = NA_REAL,
    const Rcpp::NumericVector &gamma = 0, double accrualDuration = NA_REAL,
    double followupTime = NA_REAL, const bool fixedFollowup = false,
    const Rcpp::NumericVector &spendingTime = NA_REAL,
  ...
  ListCpp out = nbsamplesize1scpp(
      ...
      critValues, alpha, typeAlphaSpending, parameterAlphaSpending, userAlpha,
      futBounds, futCP, futRate, typeBetaSpending, parameterBetaSpending,
      userBeta, lambdaH0, accrualT, accrualInt, pwSurvT, stratumFrac, kap, lam,
      gam, accrualDuration, followupTime, fixedFollowup, spendTime, rounding);
...
ListCpp nbpowerequivcpp(
    ...
    const std::vector<double> &criticalValues, const double alpha,
    const std::string &typeAlphaSpending, const double parameterAlphaSpending,
    const std::vector<double> &userAlphaSpending, const double rateRatioLower,
    const double rateRatioUpper, const double allocationRatioPlanned,
    const std::vector<double> &accrualTime,
    const std::vector<double> &accrualIntensity,
    const std::vector<double> &piecewiseSurvivalTime,
    const std::vector<double> &stratumFraction,
  ...
  if (rateRatioLower >= rateRatioUpper)
    throw std::invalid_argument(
        "rateRatioLower must be less than rateRatioUpper");
  if (allocationRatioPlanned <= 0.0)
    throw std::invalid_argument("allocationRatioPlanned must be positive");
  if (accrualTime[0] != 0.0)
    throw std::invalid_argument("accrualTime must start with 0");
  ...
  // --- compute maxInformation and theta via nbstat at study end ---
  ListCpp nb =
      nbstat1cpp(studyDuration1, 1.0, allocationRatioPlanned, accrualTime,
                 accrualIntensity, piecewiseSurvivalTime, stratumFraction,
                 kappa1v, kappa2v, lambda1v, lambda2v, gamma1x, gamma2x,
                 accrualDuration, followupTime, fixedFollowup, false);
  ...
  for (size_t i = 0; i < kMax - 1; ++i) {
    ...
    auto g = [&](double t) -> double {
      ListCpp nb1 =
          nbstat1cpp(t, 1.0, allocationRatioPlanned, accrualTime,
                     accrualIntensity, piecewiseSurvivalTime, stratumFraction,
                     kappa1v, kappa2v, lambda1v, lambda2v, gamma1x, gamma2x,
                     accrualDuration, followupTime, fixedFollowup, false);
    ...
    ListCpp nb_i =
        nbstat1cpp(time[i], 1.0, allocationRatioPlanned, accrualTime,
                   accrualIntensity, piecewiseSurvivalTime, stratumFraction,
                   kappa1v, kappa2v, lambda1v, lambda2v, gamma1x, gamma2x,
                   accrualDuration, followupTime, fixedFollowup, false);
  ...
  byStageResults.push_back(std::move(exposure), "exposure");
  byStageResults.push_back(std::move(time), "analysisTime");
  byStageResults.push_back(std::move(efficacyRRLower),
                           "efficacyRateRatioLower");
  byStageResults.push_back(std::move(efficacyRRUpper),
...
Rcpp::List nbpowerequiv(
  ...
  ListCpp out = nbpowerequivcpp(
      ...
      typeAlphaSpending, parameterAlphaSpending, userAlpha, rateRatioLower,
      rateRatioUpper, allocationRatioPlanned, accrualT, accrualInt, pwSurvT,
      stratumFrac, kap1, kap2, lam1, lam2, gam1, gam2, accrualDuration,
      followupTime, fixedFollowup, spendTime, studyDuration);
...
ListCpp nbsamplesizeequivcpp(
  ...
  ListCpp design = getDesignEquivcpp(
      beta, NaN, theta10, theta20, theta1, kMax, infoRates, critValues, alpha,
      asf, parameterAlphaSpending, userAlphaSpending, spendTime);
  ...
  auto info_minus_target_H1 =
      [allocationRatioPlanned, accrualTime, piecewiseSurvivalTime,
       ...
       maxInformation](double t, double accrDur, double fu,
                       const std::vector<double> &accrInt) {
        ListCpp nb = nbstat1cpp(t, 1.0, allocationRatioPlanned, accrualTime,
                                accrInt, piecewiseSurvivalTime, stratumFraction,
                                kappa1v, kappa2v, lambda1v, lambda2v, gamma1x,
                                gamma2x, accrDur, fu, fixedFollowup, false);
  ...
  if (unknown == ACC_DUR) {
    auto f = [info_minus_target_H1, followupTime,
              accrualIntensity](double x) -> double {
      return info_minus_target_H1(x + followupTime, x, followupTime,
                                  accrualIntensity);
    };
  ...
  ListCpp result = nbpowerequivcpp(
      ...
      parameterAlphaSpending, userAlphaSpending, rateRatioLower, rateRatioUpper,
...
Rcpp::List nbsamplesizeequiv(
  ...
  ListCpp out = nbsamplesizeequivcpp(
      ...
      rateRatioUpper, allocationRatioPlanned, accrualT, accrualInt, pwSurvT,
      stratumFrac, kap1, kap2, lam1, lam2, gam1, gam2, accrualDuration,
      followupTime, fixedFollowup, spendTime, rounding);
