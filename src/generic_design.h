...
double errorSpentcpp(const double t, const double error = 0.025,
                     const std::string &sf = "sfOF", const double sfpar = 0.0);
...
ExitProbResult exitprobcpp(const std::vector<double> &b,
                           const std::vector<double> &a,
                           const std::vector<double> &theta,
                           const std::vector<double> &I);

double f_pvalue(const double theta, const size_t L, const double zL,
                const std::vector<double> &b, const std::vector<double> &I);

std::vector<double>
getBoundcpp(const std::size_t k, const std::vector<double> &informationRates,
...
public:
  BoundCacheAlpha(std::size_t k, const std::vector<double> &infoRates,
                  const std::string &asf, double asfpar,
                  const std::vector<double> &userAlphaSpending,
                  const std::vector<double> &spendTime,
                  const std::vector<unsigned char> &effStopping,
                  std::size_t maxEntries = 64, int alphaPrecision = 12);
...
GetPowerResult getPower(const double alpha2, const size_t k2,
                        const std::vector<double> &critValues2,
                        const std::vector<double> &theta,
                        const std::vector<double> &Ic, const std::string &bsf2,
                        const double bsfpar2, const std::vector<double> &st2,
                        const std::vector<unsigned char> &futStopping2,
                        const std::vector<double> &wc, const double IL = 0,
                        const double thetaL = 0, const double zL = 0);

ListCpp getDesigncpp(
    const double beta, const double IMax, const double theta,
    const std::size_t kMax, const std::vector<double> &informationRates,
    const std::vector<unsigned char> &efficacyStopping,
    ...
    const std::vector<double> &spendingTime, const double varianceRatio = 1.0);

ListCpp getDesignEquivcpp(const double beta, const double IMax,
                          const double thetaLower, const double thetaUpper,
                          const double theta, const std::size_t kMax,
...
ListCpp adaptDesigncpp(double betaNew, double INew, const size_t L,
                       const double zL, const double theta, const double IMax,
                       const size_t kMax,
                       const std::vector<double> &informationRates,
                       const std::vector<unsigned char> &efficacyStopping,
                       const std::vector<unsigned char> &futilityStopping,
                       ...
                       const std::vector<double> &futilityTheta,
                       const std::vector<double> &spendingTime,
                       const bool MullerSchafer, const size_t kNew,
...
ListCpp getDesign2cpp(
    const double beta, const double IMax, const double theta, const size_t kMax,
    const std::vector<double> &informationRates,
    const std::vector<unsigned char> &efficacyStopping,
    const std::vector<unsigned char> &futilityStopping,
    const std::vector<double> &criticalValues, const double alpha,
    const std::string &typeAlphaSpending, const double parameterAlphaSpending,
    const std::vector<double> &userAlphaSpending, const bool symmetricBounds,
    const double astar, const std::vector<double> &futilityBounds,
    const std::string &typeBetaSpending, const double parameterBetaSpending,
    const std::vector<double> &userBetaSpending,
    const std::vector<double> &spendingTime, const double varianceRatio = 1.0);
