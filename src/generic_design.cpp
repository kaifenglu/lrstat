...
ListCpp getDesigncpp(const double beta,
  ...
  if (unknown == "IMax") {
    ...
    auto f = [&](double aval)->double {
      std::vector<double> delta = std::vector<double>(K, aval);
  ...
  std::vector<double> efficacyTheta(K);
  ...
  std::vector<double> cpuH0(K), cplH0(K);
