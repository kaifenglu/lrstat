...
// ---- Precomputed sigma context (Rec 2/3) ---------------------------------//
//                                                                            //
...
static ListCpp exitprob_mams_impl(
  ...
  if (rho == 0 || M == 1) {
    ...
    // Rec 4: Detect when every stage has identical bounds across all arms.
    ...
    if (uniform_arms) {
      for (size_t k = 0; k < kMax && uniform_arms; ++k) {
        const double ref_u = upper[k * M];
        const double ref_l = lower[k * M];
        for (size_t m = 1; m < M; ++m) {
          if (upper[k * M + m] != ref_u || lower[k * M + m] != ref_l) {
            uniform_arms = false;
            break;
          }
        }
    ...
    for (size_t k = 1; k <= kMax; ++k) {
      ...
      for (std::uint64_t i = 0; i < totalComb; ++i) {
        ...
        double pupper1, pupper2, plower;
        if (uniform_arms) {
          // Rec 4: All M arms share identical bounds at every stage.
          // Compute survival for arm 0 only, then raise to the power M.
          double s_upper1 = 1.0, s_upper2 = 1.0, s_lower = 1.0;
    ...
    // Rec 2/3: If a precomputed cache is supplied, sigma_full is already
    ...
    for (size_t k = 1; k <= kMax; ++k) {
      ...
      // Rec 1/2: Precompute per-stage Cholesky factors for sigma1 and sigma2,
      ...
      //
      // If a cache was supplied (Rec 2/3) the factorisations are already done;
      ...
      for (std::uint64_t i = 0; i < totalComb; ++i) {
        ...
        if (i == 0) {
          ...
          const std::uint64_t changed = prevGray ^ gray;
          const int bitPos = lowest_bit_index(changed);
          const std::size_t idx = active[bitPos];
          const bool choseB = ((gray >> bitPos) & 1ULL) != 0ULL;
          size_t j = idx * M;
        ...
        if (k > 1) {
          ...
          // Rec 1: use cached Cholesky for sigma2 when available (k > 2);
        ...
        // Rec 1: use cached Cholesky for sigma1 when available (k > 1);
...
std::vector<double> getBound_mams_cpp(
  ...
  // Rec 2: Precompute sigma and per-stage Cholesky factors once for the full
...
ListCpp getPower_mams(
  ...
  // Rec 3: I2 is fixed for the entire getPower_mams call.  Precompute sigma
...
ListCpp getDesign_mams_cpp(
  ...
  if (unknown == "IMax") {
    ...
    // Rec 3: Cache is rebuilt per outer Brent step (information changes
