...
inline std::vector<size_t> validateConvertIdx(
    const std::vector<int>& indices, int max_val, const char* name) {
  std::vector<size_t> result;
  result.reserve(indices.size());
  for (int idx : indices) {
    if (idx <= 0 || idx > max_val) {
      throw std::invalid_argument(std::string(name) + " contains invalid indices");
    }
