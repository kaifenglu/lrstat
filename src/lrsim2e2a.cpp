...
//' @export
// [[Rcpp::export]]
List lrsim2e2a(const int kMax = NA_INTEGER,
  ...
  lambda1e2x(nsi), lambda2e2x(nsi), 
  gamma1e1x(nsi), gamma2e1x(nsi), 
  gamma1e2x(nsi), gamma2e2x(nsi), 
  lambda1e1d(nsi), lambda2e1d(nsi), 
  gamma1e1d(nsi), gamma2e1d(nsi);
  ...
  accruals1, accruals2, totalAccruals, 
  events1e1, events2e1, totalEventse1, 
  events1e2, events2e2, totalEventse2, 
  dropouts1e1, dropouts2e1, totalDropoutse1, 
  dropouts1e2, dropouts2e2, totalDropoutse2;
  
  double s, enrollt, u, u1, u2, time, uscore, vscore;
  
  
  // stratum information 
  IntegerVector b1(nstrata), b2(nstrata), 
  n1(nstrata), n2(nstrata), n1x(nstrata), n2x(nstrata), nt(nstrata);
  ...
  lam1e2(nintervals), lam2e2(nintervals), 
  gam1e1(nintervals), gam2e1(nintervals), 
  gam1e2(nintervals), gam2e2(nintervals);
  ...
  for (iter=0; iter<maxNumberOfIterations; iter++) {
    ...
    if (useEvents) {
      ...
      if (kMax > kMaxe1x) {
        ...
        if (kMaxe1x == 0) {
          nstages = kMax;
        } else if (analysisTime2[kMax-kMaxe1x-1] > analysisTime[kMaxe1x-1]) {
    ...
    for (k=0; k<nstages; k++) {
      ...
      for (int endpoint=1; endpoint<=2; endpoint++) {
        ...
        for (i=0; i<nsub; i++) {
          ...
          if (eventSorted[i]) { 
            uscore += (treatmentGroupSorted[i]==1) - n1[h]/(nt[h]+0.0);
            vscore += n1[h]*n2[h]/(nt[h]*nt[h]+0.0);
          }
