...
struct FlatMatrix {
  ...
  FlatMatrix() = default;
  FlatMatrix(std::size_t nr, std::size_t nc)
      : data(nr * nc), nrow(nr), ncol(nc) {}

  FlatMatrix(std::vector<double> &&d, std::size_t nr, std::size_t nc)
      : data(std::move(d)), nrow(nr), ncol(nc) {
    if (nr * nc != data.size())
      throw std::runtime_error(
          "FlatMatrix: data size mismatch with dimensions");
  }
...
struct IntMatrix {
  ...
  IntMatrix() = default;
  IntMatrix(std::size_t nr, std::size_t nc)
      : data(nr * nc), nrow(nr), ncol(nc) {}
  ...
  inline int *data_ptr() noexcept {
    return data.empty() ? nullptr : data.data();
  }

  // Print helper for IntMatrix: pretty-print a small view of the matrix to
  // an ostream (default std::cout)
  inline void print(std::ostream &os = std::cout, std::size_t max_rows = 10,
                    std::size_t max_cols = 10) const {
...
//
// SztMatrix: contiguous column-major std::size_t matrix representation
// (std::size_t)
//
struct SztMatrix {
  std::vector<std::size_t> data; // column-major:
  // element (r,c) => data[c * nrow + r]
  std::size_t nrow = 0;
  std::size_t ncol = 0;

  SztMatrix() = default;
  SztMatrix(std::size_t nr, std::size_t nc)
      : data(nr * nc), nrow(nr), ncol(nc) {}
  ...
  inline std::size_t *data_ptr() noexcept {
    return data.empty() ? nullptr : data.data();
  }

  // Print helper for SztMatrix: pretty-print a small view of the matrix to
  // an ostream (default std::cout)
  inline void print(std::ostream &os = std::cout, std::size_t max_rows = 10,
                    std::size_t max_cols = 10) const {
...
//
struct BoolMatrix {
  std::vector<unsigned char> data; // column-major:
  // element(r,c) => data[c * nrow + r]
  std::size_t nrow = 0;
  std::size_t ncol = 0;

  BoolMatrix() = default;
  BoolMatrix(std::size_t nr, std::size_t nc)
      : data(nr * nc), nrow(nr), ncol(nc) {}
  ...
  BoolMatrix(std::vector<unsigned char> &&d, std::size_t nr, std::size_t nc)
    ...
    if (nr * nc != data.size())
      throw std::runtime_error(
          "BoolMatrix: data size mismatch with dimensions");
  }
  ...
  // Print helper for BoolMatrix: pretty-print a small view of the matrix to
  // an ostream Values printed as 0/1. (default std::cout)
  inline void print(std::ostream &os = std::cout, std::size_t max_rows = 10,
                    std::size_t max_cols = 10) const {
...
struct FlatArray {
  ...
  FlatArray(std::vector<double> &&d, std::size_t nr, std::size_t nc,
            std::size_t ns)
  ...
  inline void fill(double v) { std::fill(data.begin(), data.end(), v); }
  inline bool empty() const noexcept {
    return data.empty() || nrow == 0 || ncol == 0 || nslice == 0;
  }
  inline std::size_t size() const noexcept { return data.size(); }

  // index helper: (row, col, slice)
  inline static std::size_t idx(std::size_t row, std::size_t col,
                                std::size_t slice, std::size_t nrows,
...
struct DataFrameCpp {
  ...
  std::size_t nrows() const {
    if (!names_.empty()) {
      ...
      if (numeric_cols.count(nm))
        return numeric_cols.at(nm).size();
      if (int_cols.count(nm))
        return int_cols.at(nm).size();
      if (bool_cols.count(nm))
        return bool_cols.at(nm).size();
      if (string_cols.count(nm))
        return string_cols.at(nm).size();
      if (size_t_cols.count(nm))
        return size_t_cols.at(nm).size();
    }
    return 0;
  }
  ...
  void push_back(const std::vector<unsigned char> &col,
  ...
  // create p new columns named base_name, base_name.1, ..., base_name.p (if
  // p>1)
  void push_back_flat(const std::vector<double> &flat_col_major,
                      std::size_t nrows, const std::string &base_name);
  ...
  void push_front(const std::vector<unsigned char> &col,
                  const std::string &name);
  ...
  template <typename T> std::vector<T> &get(const std::string &name) {
    if constexpr (std::is_same_v<T, double>) {
      if (numeric_cols.count(name))
        return numeric_cols.at(name);
    } else if constexpr (std::is_same_v<T, int>) {
      if (int_cols.count(name))
        return int_cols.at(name);
    } else if constexpr (std::is_same_v<T, unsigned char>) {
      if (bool_cols.count(name))
        return bool_cols.at(name);
    } else if constexpr (std::is_same_v<T, std::string>) {
      if (string_cols.count(name))
        return string_cols.at(name);
    } else if constexpr (std::is_same_v<T, std::size_t>) {
      if (size_t_cols.count(name))
        return size_t_cols.at(name);
    }
    throw std::runtime_error("Column '" + name +
...
struct ListCpp {
  ...
  ska::flat_hash_map<
      std::string,
      std::variant<double, int, bool, std::string, std::size_t,
                   std::vector<double>, std::vector<int>,
                   std::vector<unsigned char>, std::vector<std::string>,
                   std::vector<std::size_t>, FlatMatrix, IntMatrix, SztMatrix,
                   BoolMatrix, FlatArray, DataFrameCpp,
  ...
  template <typename T> void push_back(T value, const std::string &name) {
  ...
  // push_front variants
  template <typename T> void push_front(T value, const std::string &name) {
    if (containsElementNamed(name))
      throw std::runtime_error("Element '" + name + "' already exists.");
    data.emplace(name, std::move(value));
...
void subset_in_place_flatmatrix(FlatMatrix &fm,
                                const std::vector<std::size_t> &row_idx);
FlatMatrix subset_flatmatrix(const FlatMatrix &fm, std::size_t start,
                             std::size_t end);
void subset_in_place_flatmatrix(FlatMatrix &fm, std::size_t start,
                                std::size_t end);
FlatMatrix subset_flatmatrix(const FlatMatrix &fm, std::size_t row_start,
                             std::size_t row_end, std::size_t col_start,
                             std::size_t col_end);
FlatArray subset_flatarray(const FlatArray &fa,
                           const std::vector<std::size_t> &row_idx);
void subset_in_place_flatarray(FlatArray &fa,
                               const std::vector<std::size_t> &row_idx);
...
std::vector<std::size_t> sztmatrix_get_column(const SztMatrix &M,
                                              std::size_t col);
std::vector<unsigned char> boolmatrix_get_column(const BoolMatrix &M,
                                                 std::size_t col);
void flatmatrix_set_column(FlatMatrix &M, std::size_t col,
                           const std::vector<double> &src);
void intmatrix_set_column(IntMatrix &M, std::size_t col,
...
template <> inline SEXP wrap(const ListCpp &l) { return convertListCppToR(l); }
template <> inline SEXP wrap(const FlatMatrix &fm) {
  if (fm.nrow == 0 || fm.ncol == 0)
    return R_NilValue;
...
template <> inline SEXP wrap(const FlatArray &fa) {
  if (fa.nrow == 0 || fa.ncol == 0 || fa.nslice == 0)
    return R_NilValue;
  ...
  Rcpp::IntegerVector dims =
      Rcpp::IntegerVector::create(fa.nrow, fa.ncol, fa.nslice);
  vec.attr("dim") = dims;
  return vec;
}
} // namespace Rcpp
