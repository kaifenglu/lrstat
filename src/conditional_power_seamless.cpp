...
std::vector<double> getCP_seamless_cpp(
    ...
    const size_t L, const double zL, const double theta, const double IMax,
    const size_t K, const std::vector<double> &informationRates,
...
// [[Rcpp::export]]
Rcpp::NumericVector getCP_seamless_Rcpp(
  ...
  auto result = getCP_seamless_cpp(
      INew, static_cast<size_t>(M), r, corr_known, static_cast<size_t>(L), zL,
      theta, IMax, static_cast<size_t>(K), infoRates, effStopping, futStopping,
