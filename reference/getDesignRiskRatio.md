...
power for a group sequential design for two-sample risk ratio.
...
- `byStageResults`: A data frame containing the following variables:
  ...
  - `efficacyRiskRatio`: The efficacy boundaries on the risk ratio
    scale.

  - `futilityRiskRatio`: The futility boundaries on the risk ratio
    scale.
...
the response probability for the control group. For statistical
inference, the parameter is often transformed to the log scale:
\$\$\theta = \log(\rho) = \log(\pi_1) - \log(\pi_2)\$\$ The variance of
the estimator \\\hat{\theta}\\ can be derived from the binomial
distributions as follows: \$\$Var(\hat{\theta}) = \frac{1}{n} \\
\frac{1-\pi_1}{\pi_1 r} + \frac{1-\pi_2}{\pi_2(1-r)} \\\$\$ where \\n\\
...
(design1 <- getDesignRiskRatio(
  ...
  alpha = 0.05))
#>                                                                                  
#> Fixed design for two-sample risk ratio                                           
#> Risk ratio under H0: 1, proportion on treatment: 0.5, proportion on control: 0.3 
#> Overall power: 0.9008, overall alpha (1-sided): 0.05                             
#> Drift parameter: 2.846, inflation factor: 1                                      
#> Information: 31.05                                                               
...
#> Efficacy boundary (risk ratio) 1.323 
