...
Simulate multi-arm group-sequential trials using a weighted log-rank
test. Analyses can be triggered either by the cumulative number of
events (combined for an active arm and the common control) or by
...
- `sumdata1`: Data frame summarizing each iteration, stage, and
  ...
  - `iterationNumber`, `eventsNotAchieved`, `stageNumber`,
...
- `summdata2`: Data frame summarizing log-rank statistics by iteration,
  ...
  - `iterationNumber`, `stageNumber`, `analysisTime`, `activeArm`,
    `totalAccruals`, `totalEvents`, `totalDropouts`, `uscore`, `vscore`,
    `logRankStatistic`.
...
- `rawdata` (present when `maxNumberOfRawDatasetsPerStage` \> 0):
  ...
  - `iterationNumber`, `stageNumber`, `analysisTime`, `subjectId`,
    `arrivalTime`, `stratum`, `treatmentGroup`, `survivalTime`,
    `dropoutTime`, `timeUnderObservation`, `event`, `dropoutEvent`.
