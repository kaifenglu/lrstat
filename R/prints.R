...
print.adaptDesign <- function(x, ...) {
  ...
  str3 = paste0("Interim adaptation look: ",  des3$L, ", ",
                "z-statistic value: ", round(des3$zL, 3))
  df3a = data.frame(x = rep("", 4))
  colnames(df3a) = NULL
  rownames(df3a) = c(str1, str2, str3, "")
...
print.adaptDesign_mams <- function(x, ...) {
  ...
  str1 <- "Primary trial:"
  str2 <- paste0("Multi-arm multi-stage design with ", des1$M,
                 " active arm(s) and ", des1$kMax, " look(s)")

  str3 <- paste0("Randomization ratio of each active vs. control: ", des1$r)

  str4 <- paste0("Using correlation for critical value calculation: ",
                 des1$corr_known)

  str5 <- paste0("Max information for pairwise comparion: ",
                 round(des1$maxInformation, 2))

  str6 <- paste0("Interim adaptation look: ", des1$L, ", ",
                 "z-statistic value: ", paste(round(des1$zL, 3), collapse = ", "))

  str7 <- paste0("Conditional type I error: ", round(des1$conditionalAlpha, 4),
                 ", conditional power: ", round(des1$conditionalPower, 3))
  ...
  # overall results
  str1 <- "Secondary trial:"
  str2 <- paste0("Multi-arm multi-stage design with ", des2$M,
                 " active arm(s) and ", des2$kMax, " look(s)")
  str3 <- paste0("Selected active arms: ", paste(des2$selected, collapse = ", "))

  str4 <- paste0("Randomization ratio of each active vs. control: ", des2$r)

  str5 <- paste0("theta: ", paste(round(des1$theta, 3), collapse = ", "),
                 ", maximum information: ", round(des2$maxInformation, 2))
  ...
  df2c[3] <- lapply(df2c[3], formatC, format = "f", digits = 3)
  colnames(df2c) <- c("Hypothesis", "Stage", "Boundary (Z)")
  ...
  str2 <- paste0("Adaptive design with ", des3$M,
                 " active arm(s) before and ", des3$MNew,
                 " active arm(s) after adaptation")

  str3 <- paste0("Selected active arms after adaptation: ",
